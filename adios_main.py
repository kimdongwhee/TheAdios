...
with open("./useData/Busan/Busan_Final_PJT.pdf", "rb") as pdf_file:
    PDFbyte = pdf_file.read()
st.download_button(label="PDF 다운로드",
                data=PDFbyte,
                file_name="Busan_Final_PJT.pdf",
                mime='application/octet-stream')
st.video("./useData/Busan/busan_video.mp4")

st.markdown("(2) 해외 축구팬들을 위한 OOF(Oracle Of Football) 플랫폼")
with open("./useData/OOF/OOF_Final_PJT.pdf", "rb") as pdf_file:
    PDFbyte = pdf_file.read()
st.download_button(label="PDF 다운로드",
                data=PDFbyte,
                file_name="OOF_Final_PJT.pdf",
                mime='application/octet-stream')
st.video("./useData/OOF/Final_OOF_VIDEO.mp4")